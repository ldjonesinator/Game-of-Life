...
{
	GetAspectRatio();
	float size_x = m_ResX * m_ZoomLevel;
	float size_y = m_ResY * m_ZoomLevel;
	m_Camera.SetProjMat(0.0f, size_x, 0.0f, size_y);
	// m_Camera.SetProjMat(0.0f, m_ResX, 0.0f, m_ResY);
...
void CameraControl::MoveCamera(bool isHorz, int direction, Timestep ts)
	{
		if (isHorz) {
			...
			// m_CameraPosition.x = (m_CameraPosition.x < m_ResX * m_LwrBndMult) ? 0 : m_CameraPosition.x;
			// m_CameraPosition.x = (m_CameraPosition.x >= m_UprBoundX / 2) ? m_UprBoundX / 2 : m_CameraPosition.x;
		} else {
			m_CameraPosition.y += m_CameraSpeed * ts * direction * m_ZoomLevel;
			// m_CameraPosition.y = (m_CameraPosition.y < m_UprBoundY * m_LwrBndMult) ? 0 : m_CameraPosition.y;
			// m_CameraPosition.y = (m_CameraPosition.y >= m_UprBoundY /  2) ? m_UprBoundY / 2 : m_CameraPosition.y;
		}
	}
...
void CameraControl::ZoomCamera(int zoomLevel)
{
	...
	m_Camera.SetProjMat(-m_AspectRatio * m_ZoomLevel / 2, m_AspectRatio * m_ZoomLevel / 2, -m_ZoomLevel, m_ZoomLevel);
